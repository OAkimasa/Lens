...
class GenerateRays:
    ...
    def generateRayLens0(self):
        ...
        # 始点を生成する
        pointsX = np.array([centerX]*25)
        pointsY, pointsZ = np.meshgrid(
            np.arange(-2+centerY, 3+centerY, 1),
            np.arange(-2+centerZ, 3+centerZ, 1))
        pointsY = pointsY.reshape(25)*rayDensity
        pointsZ = pointsZ.reshape(25)*rayDensity
    ...
    def focusFocus(self, func):
        func()
        # グラフの見た目について
        ax.set_xlim(-LX/4+focusX, LX/4+focusX)
        ax.set_ylim(-LY/4, LY/4)
        ax.set_zlim(-LZ/4, LZ/4)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        plt.title('Focus')
